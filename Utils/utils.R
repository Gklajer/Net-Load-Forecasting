...
time_cv <- function(mod_cls, eq, data, trainset_size, testset_size=1, nb_eval=1, step=0, type="rolling", is.rq=F, quant_fun=quantile, quant_p=.95, ...) {
  ...
  for(i in 1:nb_eval) {
    ...
    cv$res$pb[i] <-  pinball_loss(test_data[,as.character(eq[[2]])], 
                                  result$pred + cv$res$quant[i], quant_p, 
                                  output.vect = T)
  }
